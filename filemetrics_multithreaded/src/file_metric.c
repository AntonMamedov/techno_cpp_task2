...
//на надое дерева
typedef struct{
    HashMap* global_word_map;  //Указатель на таблицу подсчета встречаемости слова во всех файлах
...
//Структура, передаваемая в потоки
typedef struct{
    DirMetricData* data;
    char* dir_path;               //путь
    pthread_barrier_t* barrier;
    size_t start_block_index;      //откуда начинать
    size_t block_size;             //диапозон
}ThreadWorkData;
...
int  get_tf_metric_for_file(const char *dir_path, FileWordData* data, HashMap *global_word_map);
        ...
        for (size_t i = start_index_in_vec; i < start_index_in_vec + block_size; i++) {
            ...
            if (file_data != NULL) {
                ...
                if (get_tf_metric_for_file(dir_path, file_data, &metric_data->global_word_map) == -1) {
                    pthread_mutex_lock(&metric_data->mutex);
                    vec_push(strdup(file_data->file_name), &metric_data->file_error_vec);
                    pthread_mutex_unlock(&metric_data->mutex);
                }
            }
        }
    }
            ...
            if (file_data->word_list_size != 0) {
                ...
                for (size_t j = 0; j < file_data->word_list_size; j++) {
                    double tf_idf = file_data->word_list[j].tf *
                                    log10((double) (metric_data->file_info_vec.size -
                                                    metric_data->file_error_vec.size) /
...
void word_list_filling(StrMapData* data, WordListFillingData* work_data){
    ...
    StrMapData* global_tre_node_data = hash_map_insert_or_search_and_inc(str_get(&data->key),
                                                                         1, work_data->global_word_map, &flag);
...
int get_metric_for_dir(char *dir_path, DirMetricData *metric_data) {
    if (dir_path == NULL || metric_data == NULL)
    ...
    else if (metric_data->file_error_vec.array != NULL ||
        ...
        if (parse_dir(dir_path, &metric_data->file_info_vec) == -1)
            ...
            if (metric_data->file_info_vec.size < MIN_FILE_NUM_IN_DIR){
                ...
                vec_init(&thread_data_vec);
                pthread_barrier_t barrier;
                double block_size = (double)metric_data->file_info_vec.size / (double)metric_data->thread_num;
                size_t thread_block_start_index = 0;
                if (block_size < 1.0) {
                    pthread_barrier_init(&barrier, NULL, metric_data->file_info_vec.size);
                    for (size_t i = 0; i < metric_data->file_info_vec.size; i++){
                        ThreadWorkData* work_data = (ThreadWorkData*)malloc(sizeof(ThreadWorkData));
                        work_data->data = metric_data;
                        work_data->dir_path = dir_path;
                        work_data->barrier = &barrier;
                        work_data->start_block_index = thread_block_start_index;
                        work_data->block_size = 1;
                        vec_push(work_data, &thread_data_vec);
                        pthread_t* thread = (pthread_t*)malloc(sizeof(pthread_t));
                        pthread_create(thread, NULL, thread_worker, work_data);
                        vec_push(thread, &thread_vec);
                        thread_block_start_index+=1;
                    }
                }
                else{
                    pthread_barrier_init(&barrier, NULL, metric_data->thread_num);
                    for (size_t i = 0; i < metric_data->thread_num; i++){
                        ThreadWorkData* work_data = (ThreadWorkData*)malloc(sizeof(ThreadWorkData));
                        work_data->data = metric_data;
                        work_data->dir_path = dir_path;
                        work_data->barrier = &barrier;
                        work_data->start_block_index = thread_block_start_index;
                        if (i == metric_data->thread_num - 1)
                            work_data->block_size = metric_data->file_info_vec.size - thread_block_start_index;
                        else
                            work_data->block_size = (size_t)block_size;
                        vec_push(work_data, &thread_data_vec);
                        pthread_t* thread = (pthread_t*)malloc(sizeof(pthread_t));
                        pthread_create(thread, NULL, thread_worker, work_data);
                        vec_push(thread, &thread_vec);
                        thread_block_start_index+= (size_t)block_size;
                    }
                }
                for (size_t i = 0; i < thread_vec.size; i++) {
                    void** res = NULL;
                    pthread_t* thread = (pthread_t*)vec_get(i, &thread_vec);
                    pthread_join(*thread, res);
                }
                pthread_barrier_destroy(&barrier);
                vec_release(free, &thread_vec);
                vec_release(free, &thread_data_vec);
                return 0;
            }
        }
    }
...
int dir_metric_data_init(DirMetricData *data, size_t thread_num) {
    if (data == NULL)
        ...
        hash_map_init(&data->global_word_map);
...
void dir_metric_data_release(DirMetricData *data) {
    ...
    hash_map_release(&data->global_word_map);
...
void *thread_worker(void *thread_data) {
    ThreadWorkData* work_data = (ThreadWorkData*)thread_data;
    get_tf_metric_for_dir(work_data->dir_path, work_data->data, work_data->start_block_index, work_data->block_size);
    pthread_barrier_wait(work_data->barrier);
    calculate_metric_for_dir(work_data->data, work_data->start_block_index, work_data->block_size);
    return NULL;
}
